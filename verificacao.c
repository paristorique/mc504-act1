...
//estrutura dum quadro de um sudoku (têm 9)
...
//estrutura de uma linha de um sudoku
...
//funçao ligada no thread para verificar a columna
...
short int *ja = malloc(9*sizeof(short int)); //array que corresponde se já encontramos o numero considerado (de 1 a 9) na columna. ja[x]=0 significa que x (de 1 a 9)ainda nao foi encontrado, ja[x] significa o contrario
int i;
int j;
for(i=0;i<9;i++){
	ja[i]=0;}
...
//função de verificaçao de uma linha, ligada no thread
...
short int *ja = malloc(9*sizeof(short int)); //idem que para as columnas
int i;
int j;
for(i=0;i<9;i++){
	ja[i]=0;}
...
//função de verificação de um quadrinho, ligada no thread
...
short int *ja = malloc(9*sizeof(short int)); //idem columna
int i;
int j;
for(i=0;i<9;i++){
	ja[i]=0;}
...
//funcao principal de verificação
int verificar(){
